 ...
 * @param {vec3} max
 */
var BoundingBox = (MinimalGLTFLoader.BoundingBox = function (
  min,
  max,
  isClone
) {
  ...
  this.perspective =
    c.perspective === undefined
      ...
      : {
          ...
          aspectRatio:
            c.perspective.aspectRatio !== undefined
              ? c.perspective.aspectRatio
              : null,
        };
...
Node.prototype.getTransformMatrixFromTRS = function (
  translation,
  rotation,
  scale
) {
...
var Primitive = (MinimalGLTFLoader.Primitive = function (gltf, p) {
  ...
  if (this.indices !== null) {
    ...
    this.drawArraysOffset =
      gltf.json.accessors[this.attributes.POSITION].byteOffset || 0;
  }
...
var PbrMetallicRoughness = (MinimalGLTFLoader.PbrMetallicRoughness = function (
  json
) {
  this.baseColorFactor =
    json.baseColorFactor !== undefined ? json.baseColorFactor : [1, 1, 1, 1];
  this.baseColorTexture =
    json.baseColorTexture !== undefined
      ? new TextureInfo(json.baseColorTexture)
      : null;
  this.metallicFactor =
    json.metallicFactor !== undefined ? json.metallicFactor : 1;
  this.roughnessFactor =
    json.roughnessFactor !== undefined ? json.roughnessFactor : 1;
  this.metallicRoughnessTexture =
    json.metallicRoughnessTexture !== undefined
      ? new TextureInfo(json.metallicRoughnessTexture)
      : null;
...
var OcclusionTextureInfo = (MinimalGLTFLoader.OcclusionTextureInfo = function (
  json
) {
  this.index = json.index;
  this.texCoord = json.texCoord !== undefined ? json.texCoord : 0;
...
var Material = (MinimalGLTFLoader.Material = function (m) {
  ...
  this.pbrMetallicRoughness =
    m.pbrMetallicRoughness !== undefined
      ? new PbrMetallicRoughness(m.pbrMetallicRoughness)
  ...
  this.normalTexture =
    m.normalTexture !== undefined
      ? new NormalTextureInfo(m.normalTexture)
      : null;
  this.occlusionTexture =
    m.occlusionTexture !== undefined
      ? new OcclusionTextureInfo(m.occlusionTexture)
      : null;
  this.emissiveTexture =
    m.emissiveTexture !== undefined ? new TextureInfo(m.emissiveTexture) : null;

  this.emissiveFactor =
    m.emissiveFactor !== undefined ? m.emissiveFactor : [0, 0, 0];
...
var Skin = (MinimalGLTFLoader.Skin = function (gltf, s, skinID) {
  ...
  this.inverseBindMatrices =
    s.inverseBindMatrices !== undefined
      ? gltf.accessors[s.inverseBindMatrices]
      : null;
...
var SkinLink = (MinimalGLTFLoader.SkinLink = function (
  gltf,
  linkedSkin,
  ...
  this.inverseBindMatrices =
    inverseBindMatricesAccessorID !== undefined
  ...
  if (this.inverseBindMatrices) {
    ...
    for (
      var i = 0, len = this.inverseBindMatricesData.length;
      i < len;
      i += 16
    ) {
      this.inverseBindMatrix.push(
...
var AnimationSampler = (MinimalGLTFLoader.AnimationSampler = function (
  gltf,
  s
) {
  ...
  // "CUBICSPLINE"
  this.interpolation =
    s.interpolation !== undefined ? s.interpolation : "LINEAR";
...
AnimationSampler.prototype.getValue = function (t) {
  ...
  var u =
    Math.max(0, t - this.inputTypedArray[i]) /
    (this.inputTypedArray[i + 1] - this.inputTypedArray[i]);
...
glTFLoader.prototype._checkComplete = function () {
  if (
    this._bufferRequested == this._bufferLoaded &&
...
glTFLoader.prototype.load_GL_Avatar_Skin = function (
  uri,
  skeletonGltf,
  callback
) {
 ...
 */
glTFLoader.prototype.load = function (uri, callback) {
  this._init();
...
glTFLoader.prototype._postprocess = function () {
  ...
  if (this.glTF.bufferViews) {
    for (i = 0, leni = this.glTF.bufferViews.length; i < leni; i++) {
      this.glTF.bufferViews[i] = new BufferView(
        this.glTF.json.bufferViews[i],
        this._buffers[this.glTF.json.bufferViews[i].buffer]
      );
    }
  }
  ...
  if (this.glTF.accessors) {
    for (i = 0, leni = this.glTF.accessors.length; i < leni; i++) {
      this.glTF.accessors[i] = new Accessor(
        this.glTF.json.accessors[i],
        this.glTF.bufferViews[this.glTF.json.accessors[i].bufferView]
      );
    }
  }
  ...
  for (i = 0, leni = this.glTF.scenes.length; i < leni; i++) {
    scene = this.glTF.scenes[i] = new Scene(
      this.glTF,
      this.glTF.json.scenes[i]
    );
  ...
  if (this.glTF.animations) {
    for (i = 0, leni = this.glTF.animations.length; i < leni; i++) {
      this.glTF.animations[i] = new Animation(
        this.glTF,
        this.glTF.json.animations[i]
      );
    }
  }
...
// for animation use
function _arrayBuffer2TypedArray(
  buffer,
  byteOffset,
  countOfComponentType,
  componentType
) {
