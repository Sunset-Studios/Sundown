...
export class DDGI {
  ...
  _add_short_range_gi_passes(
    ...
    let final_diffuse_output = combined_indirect_diffuse_output;
    let atrous_read_texture = combined_indirect_diffuse_output;
    ...
    if (diffuse_atrous_enabled && diffuse_atrous_pass_count > 0) {
      for (let pass_index = 0; pass_index < diffuse_atrous_pass_count; pass_index += 1) {
        render_graph.add_pass(
          `ddgi_diffuse_atrous_upload_params_${ping_pong_frame}_${pass_index}`,
          RenderPassFlags.GraphLocal,
          {},
          (graph, frame_data, encoder) => {
        ...
        render_graph.add_pass(
          `ddgi_diffuse_atrous_${ping_pong_frame}_${pass_index}`,
          RenderPassFlags.Compute,
          {
            inputs: [
