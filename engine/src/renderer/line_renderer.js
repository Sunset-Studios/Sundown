...
export class LineRenderer {
     ...
     */
    static add_line(start, end, color = DEFAULT_LINE_COLOR, width = LINE_WIDTH, collection_id = null) {
        let line_position;
        let line_data;
        let index;
        ...
        const actual_collection_id = collection_id !== null ? collection_id : this.current_collection_id;
     ...
     * @param {number} collection_id - Optional collection ID (uses current collection if not specified)
     */
    static add_grid(size = 10, spacing = 1, center = [0, 0, 0], color = DEFAULT_LINE_COLOR, width = LINE_WIDTH, collection_id = null) {
     ...
     * @param {number} collection_id - Optional collection ID (uses current collection if not specified)
     */
    static add_circle(center, radius, normal = [0, 1, 0], segments = 32, color = DEFAULT_LINE_COLOR, width = LINE_WIDTH, collection_id = null) {
        ...
        for (let i = 0; i < segments; i++) {
            ...
            const x = center[0] + radius * (Math.cos(angle) * tangent[0] + Math.sin(angle) * bitangent[0]);
            const y = center[1] + radius * (Math.cos(angle) * tangent[1] + Math.sin(angle) * bitangent[1]);
            const z = center[2] + radius * (Math.cos(angle) * tangent[2] + Math.sin(angle) * bitangent[2]);
            points.push([x, y, z]);
        }
     ...
     * @param {number} collection_id - Optional collection ID (uses current collection if not specified)
     */
    static add_sphere(center, radius, rings = 8, segments = 16, color = DEFAULT_LINE_COLOR, width = LINE_WIDTH, collection_id = null) {
        ...
        for (let i = 0; i <= rings; i++) {
            ...
            this.add_circle([center[0], y, center[2]], ring_radius, [0, 1, 0], segments, color, width, collection_id);
        }
     ...
     * @param {number} collection_id - Optional collection ID (uses current collection if not specified)
     */
    static add_arrow(start, end, head_size = 0.2, color = DEFAULT_LINE_COLOR, width = LINE_WIDTH, collection_id = null) {
        // Draw the main line
        ...
        for (let i = 0; i < 4; i++) {
            ...
            const x = arrow_base[0] + head_size * 0.5 * (Math.cos(angle) * perp1[0] + Math.sin(angle) * perp3[0]);
            const y = arrow_base[1] + head_size * 0.5 * (Math.cos(angle) * perp1[1] + Math.sin(angle) * perp3[1]);
            const z = arrow_base[2] + head_size * 0.5 * (Math.cos(angle) * perp1[2] + Math.sin(angle) * perp3[2]);
    ...
    static to_gpu_data() {
        ...
        if (!this.transform_buffer || this.transform_buffer.config.size < this.last_visible_line_count * 16) {
