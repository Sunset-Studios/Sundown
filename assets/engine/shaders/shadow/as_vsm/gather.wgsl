...
@group(1) @binding(3) var<storage, read_write> requested_tiles: array<atomic<u32>>;
@group(1) @binding(4) var<storage, read> dense_lights_buffer: array<Light>;
@group(1) @binding(5) var<storage, read> dense_shadow_casting_lights_buffer: array<u32>;
@group(1) @binding(6) var<storage, read> light_count_buffer: array<u32>;
@group(1) @binding(7) var<storage, read> vsm_settings: ASVSMSettings;
...
fn cs(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    while(bits != 0u) {
      ...
      if (!pte_is_valid) {
        let max_tile_requests = u32(vsm_settings.max_tile_requests);
