...
fn cs(@builtin(global_invocation_id) id: vec3<u32>) {
  ...
  // TODO: Evicting pages the moment tiles become invisible like this is a bit too aggressive.
  // Need to change this to only evict non-feedback pages when another tile request needs this LRU entry.
  // Otherwise, we'll always be evicting pages for ofscreen objects, which will prevent shadowing from offscreen objects, which we don't want.

  if (vsm_pte_is_valid(entry)) {
    if (not_visible) {
      atomicAnd(&lru[1u + current_physical_id], ~lru_pinned_flag);
    } else {
      atomicOr(&lru[1u + current_physical_id], lru_pinned_flag);
    }
  }
