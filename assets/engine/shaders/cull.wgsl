...
fn sphere_project(center: vec4<f32>, radius: f32, p00: f32, p11: f32, aabb: ptr<function, vec4<f32>>, view: ptr<function, View>) -> bool {
    ...
    let dist = length(center_view.xyz);
    if (dist <= radius) {
        return false;
    }
    // Compute projected center in NDC.
    let ndc_center = vec2f((center_view.x / center_view.z) * -p00,
                            (center_view.y / center_view.z) * p11);
    // Compute uniform projected radius in NDC (inflate by 1.2 to be conservative).
    let ndc_radius = (radius / center_view.z) * 1.2;
    let ndc_min = ndc_center - vec2f(ndc_radius, ndc_radius);
    let ndc_max = ndc_center + vec2f(ndc_radius, ndc_radius);
    *aabb = vec4f(ndc_min.x, ndc_min.y, ndc_max.x, ndc_max.y);
...
fn is_occluded(center: vec4<f32>, radius: f32, view: ptr<function, View>) -> u32 {
    ...
    var center_view = view_matrix * center;
    center_view.z -= radius;

    let proj_point = view.projection_matrix * center_view;
    let sphere_depth = (proj_point.z / proj_point.w); 

    // Set bias proportional to view depth.
    let dist_bias_val = center_view.z * 0.000001;
...
fn cs(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (g_id < u32(draw_cull_constants.draw_count)) {
        ...
        radius *= 1.005; // Inflate bounds conservatively
