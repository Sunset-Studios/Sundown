// =============================================================================
// Path Tracer - Hit Pass (Binned Ray Processing)
// - Processes rays in binned order for improved cache coherence
// - Rays with similar directions traverse BVH together
// - Improves SIMD utilization and memory access patterns
// =============================================================================
...
// Layout: [8 x u32 counts] [8 x u32 offsets] [N x u32 ray_indices]
struct RayBinInfo {
...
@group(1) @binding(8) var<storage, read> bin_info: RayBinInfo;
@group(1) @binding(9) var output_tex: texture_storage_2d<rgba16float, write>;

// Note: mesh_asset_ids and index_buffer accessed through vertex_buffer and blas_atlas
...
// Process rays from binned indices
...
fn cs(
    ...
    // Determine which bin we're processing based on workgroup ID
    // Each bin gets processed by dedicated workgroups for maximum coherence
    let bin_id = wg_id.z;  // Use Z dimension for bin ID
    if (bin_id >= 8u) { return; }
    
    let bin_start = bin_info.bin_offsets[bin_id];
    let bin_count = atomicLoad(&bin_info.bin_counts[bin_id]);
    let ray_index_in_bin = wg_id.x * 128u + local_idx;
    
    if (ray_index_in_bin >= bin_count) { return; }
    
    let binned_index = bin_start + ray_index_in_bin;
    let pixel_index = bin_info.ray_indices[binned_index];
    ...
    // Main ray traversal (all rays in this bin have similar directions)
