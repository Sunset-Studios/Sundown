...
@group(1) @binding(4) var<storage, read> blas_atlas: BLASAtlas;                 // Unified BLAS atlas
...
@group(1) @binding(7) var<storage, read> mesh_asset_ids: array<u32>;         // Primitive -> mesh id
@group(1) @binding(8) var position_tex: texture_2d<f32>;                     // GBuffer world position
@group(1) @binding(9) var normal_tex: texture_2d<f32>;      
...
fn cs(@builtin(global_invocation_id) gid: vec3<u32>) {
	...
	for (var k = 0u; k < TLAS_CANDIDATES; k = k + 1u) {
        ...
        var entity_transform = entity_transforms[prim_store].transform;
        let blas_hit = trace_blas(&ray, &entity_transform, mesh_asset_id);
        if (blas_hit.normal_and_user_data.w >= 0.0) {
            let tri_id = u32(blas_hit.normal_and_user_data.w);
