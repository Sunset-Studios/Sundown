...
fn cs(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let center_uv = coord_to_uv(pixel_coord, resolution);
    let center_position = reconstruct_world_position(center_uv, center_depth, view_index);
    let center_view = view_buffer[view_index].view_position.xyz;
    let center_linear_depth = length(center_position - center_view);
    ...
    for (var y = -1; y <= 1; y = y + 1) {
        for (var x = -1; x <= 1; x = x + 1) {
            ...
            if (sample_depth >= 1.0 || length(sample_normal_data.xyz) <= 0.0) {
                continue;
            }

            let sample_uv = coord_to_uv(sample_coord, resolution);
            let sample_position = reconstruct_world_position(sample_uv, sample_depth, view_index);
            let sample_normal = safe_normalize(sample_normal_data.xyz);
            let sample_linear_depth = length(sample_position - center_view);
            let sample_ao = textureLoad(input_ao, sample_coord, 0).r;

            let kernel_weight = get_kernel_weight(offset);
            let depth_rel = abs(sample_linear_depth - center_linear_depth) / max(center_linear_depth, 1e-4);
