...
fn cs(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if (atomicLoad(&world_cache[cell_index].fingerprint) == WORLD_CACHE_CELL_EMPTY) {
        // Mark ray as dead
        world_cache_path_state[active_index].state_u32 = vec4<u32>(0u, 0u, 0u, 0u);
        return;
    }
    ...
    let selected_target_pdf = selected_sample.radiance_and_target_pdf.w;
    gi_reservoir_finalize(&gi_reservoir, selected_target_pdf);
    
    let ray_dir = selected_dir;
    let source_pdf = selected_sample.direction_and_source_pdf.w;
    ...
    if (num_lights > 0u) {
        ...
        world_cache_path_state[active_index].shadow_direction = vec4f(light_dir, selected_light_distance * 0.999);
    ...
    world_cache_path_state[active_index].state_u32 = vec4<u32>(0u, 1u, 0u, 0xffffffffu);
    world_cache_path_state[active_index].hit_attr0 = vec4<f32>(0.0);
    world_cache_path_state[active_index].hit_attr1 = vec4<f32>(0.0);
    world_cache_path_state[active_index].rng_rank_frame_stamp = vec4<f32>(f32(rng), f32(rank), f32(frame_id), 0.0);
    world_cache_path_state[active_index].path_weight = vec4<f32>(path_weight, source_pdf);
}
