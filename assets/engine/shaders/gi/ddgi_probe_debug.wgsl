// =============================================================================
// DDGI Probe Debug Visualization
// - Renders probe spheres with accumulated radiance
// =============================================================================
#include "common.wgsl"
#include "gi/ddgi_common.wgsl"

@group(1) @binding(0) var<uniform> ddgi_params: DDGIParams;
@group(1) @binding(1) var probe_irradiance_atlas: texture_2d<f32>;
...
// =============================================================================
// Ray Helpers (freestanding probes, no GBuffer dependency)
// =============================================================================
fn ddgi_world_ray_direction(pixel_uv: vec2<f32>, view_index: u32) -> vec3<f32> {
...
fn ddgi_ray_sphere_intersect(ro: vec3<f32>, rd: vec3<f32>, center: vec3<f32>, radius: f32) -> f32 {
...
fn ddgi_ray_aabb_intersect(ro: vec3<f32>, rd: vec3<f32>, bmin: vec3<f32>, bmax: vec3<f32>) -> vec2<f32> {
    // Slab intersection; returns (t_enter, t_exit). If miss, t_enter > t_exit.
    let inv_rd = vec3<f32>(
...
fn ddgi_world_position_from_depth(pixel_uv: vec2<f32>, depth: f32, view_index: u32) -> vec3<f32> {
    // Reconstruct world position from NDC (x,y from uv, z from depth)
    let ndc = vec2<f32>(pixel_uv.x * 2.0 - 1.0, (1.0 - pixel_uv.y) * 2.0 - 1.0);
    let inv_vp = view_buffer[view_index].inverse_view_projection_matrix;
...
fn cs(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Freestanding probe rendering:
    // - Reconstruct a camera ray per pixel
    // - Intersect that ray against probe spheres in world space
    // ─────────────────────────────────────────────────────────────────────────
    ...
    let ray_direction = ddgi_world_ray_direction(pixel_uv, view_index);

    // Probe volume bounds in world space (covers all probe vertices)
    let dims = vec3<u32>(
        u32(ddgi_params.probe_grid_dims.x),
        u32(ddgi_params.probe_grid_dims.y),
        u32(ddgi_params.probe_grid_dims.z)
    );
    let max_vertex = vec3<f32>(f32(max(dims.x, 1u) - 1u), f32(max(dims.y, 1u) - 1u), f32(max(dims.z, 1u) - 1u));
    let bmin_ws = origin;
    let bmax_ws = origin + max_vertex * spacing;

    let t_range = ddgi_ray_aabb_intersect(ray_origin, ray_direction, bmin_ws - vec3<f32>(probe_radius), bmax_ws + vec3<f32>(probe_radius));
    if (t_range.x > t_range.y) {
        textureStore(output_debug, pixel_coord, vec4f(scene, 1.0));
        return;
    }

    // Traverse cells (dims-1) and test the 8 vertices per cell for ray-sphere hits.
    ...
    // Convert ray into "grid vertex space" (units = spacing)
    let ro_vs = (ray_origin - origin) / max(spacing, 1e-8);
    let rd_vs = ray_direction / max(spacing, 1e-8);
    let vs_range = ddgi_ray_aabb_intersect(ro_vs, rd_vs, vec3<f32>(0.0), vec3<f32>(f32(cell_dims.x), f32(cell_dims.y), f32(cell_dims.z)));
    ...
    // Safety cap for debug
    let max_steps = 32u;
    for (var iter = 0u; iter < max_steps; iter = iter + 1u) {
        // Test 8 vertices of this cell
        ...
        for (var oz = 0u; oz < 2u; oz = oz + 1u) {
            for (var oy = 0u; oy < 2u; oy = oy + 1u) {
                for (var ox = 0u; ox < 2u; ox = ox + 1u) {
                    ...
                    let t = ddgi_ray_sphere_intersect(ray_origin, ray_direction, center, probe_radius);
        ...
        // Advance to next cell along the ray
    ...
    // Depth-based occlusion: if the closest opaque surface is nearer than the sphere hit, hide the sphere.
    let depth = textureSampleLevel(depth_texture, non_filtering_sampler, pixel_uv, 0.0).r;
    if (depth < 1.0) {
        let p_surface = ddgi_world_position_from_depth(pixel_uv, depth, view_index);
    ...
    let sphere_n = safe_normalize(hit_pos - probe_center);
    let sphere_color = ddgi_sample_probe_irradiance(&ddgi_params, probe_irradiance_atlas, hit_probe_index, sphere_n);

    textureStore(output_debug, pixel_coord, vec4f(sphere_color, 1.0));
}
