...
const SCGI_BUCKET_SIZE: u32 = 8u;
const SCGI_LOD_EXTENT_CELLS: f32 = 128.0;
const SCGI_NORMAL_QUANTIZATION: f32 = 1.0;
const SCGI_PATCH_EMPTY: u32 = 0u;
const SCGI_MAX_RADIANCE: f32 = 10.0;
const SCGI_SH_PATCH_SIZE_U32: u32 = 6u;
...
struct SCGIRadianceInfo {
    shadow_radiance: vec4<f32>,
    sample_radiance: vec4<f32>,
};

fn scgi_sh_patch_read(
    buffer: ptr<storage, array<u32>, read_write>,
    patch_index: u32
) -> SH_L1_RGB {
    let base_offset = patch_index * SCGI_SH_PATCH_SIZE_U32;
    var packed: SH_L1_RGB_Packed;
    for (var coefficient = 0u; coefficient < SCGI_SH_PATCH_SIZE_U32; coefficient = coefficient + 1u) {
        packed.data[coefficient] = (*buffer)[base_offset + coefficient];
    }
    return sh_l1_rgb_unpack(packed);
}

fn scgi_sh_patch_write(
    buffer: ptr<storage, array<u32>, read_write>,
    patch_index: u32,
    sh: SH_L1_RGB
) {
...
fn scgi_hemisphere_frame(normal: vec3<f32>) -> mat3x3<f32> {
    return orthonormalize(safe_normalize(normal));
}

fn scgi_world_to_hemisphere(direction: vec3<f32>, normal: vec3<f32>) -> vec3<f32> {
    return transpose(scgi_hemisphere_frame(normal)) * direction;
}

fn scgi_rotate_sh_between_hemispheres(
    sh: SH_L1_RGB,
    source_normal: vec3<f32>,
    receiver_normal: vec3<f32>
) -> SH_L1_RGB {
    let source_to_world = scgi_hemisphere_frame(source_normal);
    let world_to_receiver = transpose(scgi_hemisphere_frame(receiver_normal));
    return sh_l1_rgb_rotate(sh, world_to_receiver * source_to_world);
}

fn scgi_evaluate_local_sh_irradiance(sh: SH_L1_RGB) -> vec3<f32> {
    return max(
...
fn scgi_full_resolution(params: SCGIParams) -> vec2<u32> {
    return vec2<u32>(u32(params.full_resolution_x), u32(params.full_resolution_y));
}

fn scgi_select_lod(position: vec3<f32>, camera_position: vec3<f32>, params: SCGIParams) -> u32 {
    let delta = abs(position - camera_position);
    let square_distance = max(delta.x, max(delta.y, delta.z));
    let base_extent = max(
        params.surface_cache_cell_size * SCGI_LOD_EXTENT_CELLS,
...
fn scgi_lod_cell_size(lod: u32, params: SCGIParams) -> f32 {
    return params.surface_cache_cell_size * f32(1 << lod);
}

fn scgi_quantize_position(position: vec3<f32>, lod: u32, params: SCGIParams) -> vec3<i32> {
    return vec3<i32>(floor(position / scgi_lod_cell_size(lod, params) + vec3<f32>(0.0001)));
}
...
fn scgi_world_cell_center(
    ...
    let cell_size = scgi_lod_cell_size(lod, params);
...
fn scgi_quantize_normal(normal: vec3<f32>) -> vec2<i32> {
    ...
    return vec2<i32>(floor(oct * SCGI_NORMAL_QUANTIZATION + vec2<f32>(0.5)));
}

fn scgi_bucket_start(position: vec3<i32>, normal: vec2<i32>, lod: u32, params: SCGIParams) -> u32 {
    ...
    let patches_per_lod = max(u32(params.surface_cache_size), SCGI_BUCKET_SIZE);
    let bucket_count = max(patches_per_lod / SCGI_BUCKET_SIZE, 1u);
    let lod_start = lod * patches_per_lod;
    return lod_start + (combined % bucket_count) * SCGI_BUCKET_SIZE;
}

fn scgi_hash_fingerprint(position: vec3<i32>, normal: vec2<i32>, lod: u32) -> u32 {
...
fn scgi_patch_descriptor_matches(
    ...
    return
        all(scgi_quantize_position(patch_position, lod, params) == quantized_position) &&
        all(scgi_quantize_normal(patch_normal) == quantized_normal);
}

fn scgi_patch_rng(patch_index: u32, patch_fingerprint: u32) -> u32 {
