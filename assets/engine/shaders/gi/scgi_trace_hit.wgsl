...
@group(1) @binding(5) var<storage, read_write> hit_info: array<SCGIHitInfo>;
...
fn sample_uniform_hemisphere_scgi(normal: vec3<f32>, r1: f32, r2: f32) -> vec3<f32> {
    ...
    return scgi_hemisphere_frame(normal) * vec3<f32>(
        sin_theta * cos(phi),
        sin_theta * sin(phi),
        cos_theta
    );
...
fn generate_ray_sample(
    ...
    let candidate_count = u32(clamp(
        i32(scgi_params.importance_sample_count),
        2,
        16
    ));
    ...
    if (patch_history > 0.0) {
        history_scale = max(luminance(scgi_evaluate_local_sh_irradiance(previous_sh)) / PI, 1e-3);
    }
    ...
    let exploration = clamp(scgi_params.importance_exploration, 0.01, 1.0);
    ...
    for (var candidate_index = 0u; candidate_index < candidate_count; candidate_index = candidate_index + 1u) {
        ...
        let local_direction = safe_normalize(scgi_world_to_hemisphere(candidate_direction, normal));
...
fn trace_scgi_ray(active_index: u32) {
    ...
    let previous_sh = scgi_sh_patch_read(&surface_cache_sh, patch_index);
