...
@group(1) @binding(2) var<storage, read> active_indices: array<u32>;
@group(1) @binding(3) var<storage, read> counters: SCGICountersReadOnly;
@group(1) @binding(4) var<storage, read_write> hit_info: array<SCGIHitInfo>;
...
@group(1) @binding(9) var<storage, read> ray_instance_transforms: array<RayInstanceTransform>;
...
fn generate_ray_direction(seed: u32, normal: vec3<f32>, sample_index: u32) -> vec3<f32> {
    // A stable Cranley-Patterson rotation over an R2 sequence continues from
    // the patch's persistent history instead of reshuffling with frame order.
    let sequence_index = f32(sample_index);
    var rng = random_seed(seed);
    let rotation_u = rand_float(rng);
    rng = random_seed(rng);
    let rotation_v = rand_float(rng);
    let r1 = fract(rotation_u + sequence_index * 0.7548776662466927);
    let r2 = fract(rotation_v + sequence_index * 0.5698402909980532);
    return sample_uniform_hemisphere(normal, r1, r2);
}

fn trace_scgi_ray(active_index: u32) {
    ...
    let seed = scgi_patch_rng(patch_index, surface_patch.fingerprint);
    let direction = generate_ray_direction(seed, normal, u32(surface_patch.history.y));

    var ray: Ray;
    ray.origin_and_tmin = vec4<f32>(surface_patch.position_frame.xyz + normal * 0.001, 0.0001);
    ray.direction_and_tmax = vec4<f32>(direction, scgi_params.max_ray_length);
    ...
    hit_info[active_index].hit_position_t = vec4<f32>(0.0);
    ...
    hit_info[active_index].hit_position_t = vec4<f32>(
        hit_position_world,
        hit_result.t_hit
    );
