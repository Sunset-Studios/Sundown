...
fn image_alpha(entity_id: u32, uv: vec2<f32>) -> f32 {
    return image_color(entity_id, uv).a;
}

fn image_coverage_mask(position: vec4<f32>, entity_id: u32, uv: vec2<f32>) -> f32 {
    let alpha = image_alpha(entity_id, uv);
#if TRANSPARENT
    return alpha;
#else
...
fn depth_fragment_mask(input: DepthVertexOutput) -> f32 {
    return image_coverage_mask(input.position, input.entity_id, input.uv);
}
#endif
...
fn forward_fragment(
    ...
    f_out.color = image_color(input.entity_id, input.uv);
    return *f_out;
}
#endif
...
fn resolve_fragment(
    ...
    let image = image_color(input.entity_id, input.uv);
